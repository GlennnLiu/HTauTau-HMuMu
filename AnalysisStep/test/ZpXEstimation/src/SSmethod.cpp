...
//       _TauIDSF = calculate_TauIDSF(input_file_data_name,false);
...
//       }
      ...
      else if ( PFMET > 100000000000000000 ) {//(fabs(LepLepId->at(2)) == 11) ? _failMETCut[Settings::ele]++ : _failMETCut[Settings::mu]++; continue;}
          ...
          if(LepisID->at(2) && ((abs(LepLepId->at(2)) == 11) ? LepCombRelIsoPF->at(2) < 999999. : LepCombRelIsoPF->at(2) < 0.35))
            {
              ...
              if(abs(LepLepId->at(2)) == 11 ) passing[_current_process][Settings::ele][0]->Fill(LepPt->at(2), PFMET, (fabs(LepEta->at(2)) < 1.479) ? 0.5 : 1.5 , (_current_process == Settings::Data) ? 1 :  _event_weight);
              else if(abs(LepLepId->at(2)) == 13 ) passing[_current_process][Settings::mu][0]->Fill(LepPt->at(2), PFMET, (fabs(LepEta->at(2)) < 1.2) ? 0.5 : 1.5 , (_current_process == Settings::Data) ? 1 :  _event_weight);
            }
          else
            {
              (abs(LepLepId->at(2)) == 11) ? _faillingSelection[Settings::ele]++ : _faillingSelection[Settings::mu]++;
              if(abs(LepLepId->at(2)) == 11 ) failing[_current_process][Settings::ele][0]->Fill(LepPt->at(2), PFMET, (fabs(LepEta->at(2)) < 1.479) ? 0.5 : 1.5 , (_current_process == Settings::Data) ? 1 :  _event_weight);
              else if(abs(LepLepId->at(2)) == 13 ) failing[_current_process][Settings::mu][0]->Fill(LepPt->at(2), PFMET, (fabs(LepEta->at(2)) < 1.2) ? 0.5 : 1.5 , (_current_process == Settings::Data) ? 1 :  _event_weight);
            }
        }
      else if (LepisID->at(2))// if (abs(LepLepId->at(2))==15)
	{
      double decayID=-1;
      if (TauDecayMode->at(2)==0) decayID=0.5;
      else if (TauDecayMode->at(2)==1) decayID=1.5;
      else if (TauDecayMode->at(2)==10) decayID=2.5;
      else if (TauDecayMode->at(2)==11) decayID=3.5;
      _afterSel_events[Settings::tauE]++;_afterSel_events[Settings::tauMu]++;_afterSel_events[Settings::tauTau]++;
	  _k_factor = calculate_K_factor(input_file_data_name);
	  _event_weight = (_lumi * 1000 * xsec * _k_factor * overallEventWeight * L1prefiringWeight) / gen_sum_weights;

	  if (TauVSmu->at(2)>=4 && TauVSe->at(2)>=3 && TauVSjet->at(2)>=6) {
	    _passingSelection[Settings::tauMu]++;_passingSelection[Settings::tauTau]++;
	    _TauIDSF=calculate_TauIDSF_OneLeg(TauDecayMode->at(2), LepPt->at(2), LepEta->at(2), GENMatch(2,input_file_data_name), 225, true);
	    passing[_current_process][Settings::tauMu][0]->Fill(LepPt->at(2), PFMET, decayID, (_current_process == Settings::Data) ? 1 :  _event_weight*_TauIDSF);
	    passing[_current_process][Settings::tauTau][0]->Fill(LepPt->at(2), PFMET, decayID, (_current_process == Settings::Data) ? 1 :  _event_weight*_TauIDSF);
	  }
	  else {
	    _faillingSelection[Settings::tauMu]++;_faillingSelection[Settings::tauTau]++;
            _TauIDSF=calculate_TauIDSF_OneLeg(TauDecayMode->at(2), LepPt->at(2), LepEta->at(2), GENMatch(2,input_file_data_name), 225, false);
            failing[_current_process][Settings::tauMu][0]->Fill(LepPt->at(2), PFMET, decayID, (_current_process == Settings::Data) ? 1 :  _event_weight*_TauIDSF);
            failing[_current_process][Settings::tauTau][0]->Fill(LepPt->at(2), PFMET, decayID, (_current_process == Settings::Data) ? 1 :  _event_weight*_TauIDSF);
          }
	  if (TauVSmu->at(2)>=4 && TauVSe->at(2)>=5 && TauVSjet->at(2)>=5) {
	    _passingSelection[Settings::tauE]++;
	    _TauIDSF=calculate_TauIDSF_OneLeg(TauDecayMode->at(2), LepPt->at(2), LepEta->at(2), GENMatch(2,input_file_data_name), 165, true);
	    passing[_current_process][Settings::tauE][0]->Fill(LepPt->at(2), PFMET, decayID, (_current_process == Settings::Data) ? 1 :  _event_weight*_TauIDSF);
	  }
	  else {
            _faillingSelection[Settings::tauE]++;
            _TauIDSF=calculate_TauIDSF_OneLeg(TauDecayMode->at(2), LepPt->at(2), LepEta->at(2), GENMatch(2,input_file_data_name), 165, false);
            failing[_current_process][Settings::tauE][0]->Fill(LepPt->at(2), PFMET, decayID, (_current_process == Settings::Data) ? 1 :  _event_weight*_TauIDSF);
          }
...
/*
  ...
  */    
      ...
      // Calculate yield
      _yield_SR    = _fs_ROS_SS.at(_current_final_state)*FR->GetFakeRate(LepPt->at(2),PFMET,nCleanedJets,LepEta->at(2),TauDecayMode->at(2),LepLepId->at(2),tauChannel)*FR->GetFakeRate(LepPt->at(3),PFMET,nCleanedJets,LepEta->at(3),TauDecayMode->at(3),LepLepId->at(3),tauChannel);
      _yield_SR_up = _fs_ROS_SS.at(_current_final_state)*FR->GetFakeRate_Up(LepPt->at(2),PFMET,nCleanedJets,LepEta->at(2),TauDecayMode->at(2),LepLepId->at(2),tauChannel)*FR->GetFakeRate_Up(LepPt->at(3),PFMET,nCleanedJets,LepEta->at(3),TauDecayMode->at(3),LepLepId->at(3),tauChannel);
      _yield_SR_dn = _fs_ROS_SS.at(_current_final_state)*FR->GetFakeRate_Dn(LepPt->at(2),PFMET,nCleanedJets,LepEta->at(2),TauDecayMode->at(2),LepLepId->at(2),tauChannel)*FR->GetFakeRate_Dn(LepPt->at(3),PFMET,nCleanedJets,LepEta->at(3),TauDecayMode->at(3),LepLepId->at(3),tauChannel);
...
void SSmethod::DeclareFRHistos()
{
   for (int i_flav = 0; i_flav < num_of_flavours; i_flav++)
   {
      ...
      {
          for (int i = 0; i < 3; i++) {
	_histo_name = "Passing_" + _s_process.at(i_proc) + "_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
	passing[i_proc][i_flav][i] = new TH3F(_histo_name,"", 80, 0, 80, 100, 0, 100, 4, 0, 4);
        
	_histo_name = "Failing_" + _s_process.at(i_proc) + "_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
	failing[i_proc][i_flav][i] = new TH3F(_histo_name,"", 80, 0, 80, 100, 0, 100, 4, 0, 4);
          }
      }
      
       for (int i = 0; i < 3; i++) {
      _histo_name = "Passing_Total_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
      passing[Settings::Total][i_flav][i] = new TH3F(_histo_name,"", 80, 0, 80, 100, 0, 100, 4, 0, 4);
      _histo_name = "Failing_Total_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
      failing[Settings::Total][i_flav][i] = new TH3F(_histo_name,"", 80, 0, 80, 100, 0, 100, 4, 0, 4);
       }
   }
...
void SSmethod::SaveFRHistos( TString file_name, bool subtractWZ, bool remove_negative_bins)
{
    ...
    {
        for (int i = 0; i < 3; i++) {
        passing[Settings::Total][i_flav][i]->Add(passing[Settings::Data][i_flav][i], 1.);
        failing[Settings::Total][i_flav][i]->Add(failing[Settings::Data][i_flav][i], 1.);
        }
    }
    ...
    if ( remove_negative_bins ) // Set negative bins to zero
    {
        ...
        {
            for (int i = 0; i < 3; i++) {
                RemoveNegativeBins3D( passing[Settings::Total][i_flav][i] );
                RemoveNegativeBins3D( failing[Settings::Total][i_flav][i] );
            }
        }
    ...
    for (int i_flav = 0; i_flav < num_of_flavours; i_flav++)
    {
        ...
        {
            for (int i = 0; i < 3; i++) {
                passing[i_proc][i_flav][i]->Write();
                failing[i_proc][i_flav][i]->Write();
            }
        }
        for (int i = 0; i < 3; i++) {
            passing[Settings::Total][i_flav][i]->Write();
            failing[Settings::Total][i_flav][i]->Write();
        }
    }
...
void SSmethod::GetFRHistos( TString file_name)
{
    ...
    for (int i_flav = 0; i_flav < num_of_flavours; i_flav++) {
        for (int i_proc = 0; i_proc < Settings::Total; i_proc++) {
            for (int i = 0; i < 3; i++) {
                _histo_name = "Passing_" + _s_process.at(i_proc) + "_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
                passing[i_proc][i_flav][i] = (TH3F*)histo_file->Get(_histo_name);

                _histo_name = "Failing_" + _s_process.at(i_proc) + "_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
                failing[i_proc][i_flav][i] = (TH3F*)histo_file->Get(_histo_name);
            }
        }

        for (int i = 0; i < 3; i++) {
            _histo_name = "Passing_Total_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
            passing[Settings::Total][i_flav][i] = (TH3F*)histo_file->Get(_histo_name);
            _histo_name = "Failing_Total_" + _s_flavour.at(i_flav) + "_" + std::to_string(i);
            failing[Settings::Total][i_flav][i] = (TH3F*)histo_file->Get(_histo_name);
        }
    }
...
{
    FR_SS_electron_EB=new TH1F("FR_SS_electron_EB",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    FR_SS_electron_EE=new TH1F("FR_SS_electron_EE",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    FR_SS_muon_EB=new TH1F("FR_SS_muon_EB",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    FR_SS_muon_EE=new TH1F("FR_SS_muon_EE",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    for (int i = 0; i < 1; i++) {
        FR_SS_tauE_Decay0[i]= new TH2F(Form("FR_SS_tauE_Decay0_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauE_Decay1[i]= new TH2F(Form("FR_SS_tauE_Decay1_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauE_Decay10[i]= new TH2F(Form("FR_SS_tauE_Decay10_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauE_Decay11[i]= new TH2F(Form("FR_SS_tauE_Decay11_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay0[i]= new TH2F(Form("FR_SS_tauMu_Decay0_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay1[i]= new TH2F(Form("FR_SS_tauMu_Decay1_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay10[i]= new TH2F(Form("FR_SS_tauMu_Decay10_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay11[i]= new TH2F(Form("FR_SS_tauMu_Decay11_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay0[i]= new TH2F(Form("FR_SS_tauTau_Decay0_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay1[i]= new TH2F(Form("FR_SS_tauTau_Decay1_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay10[i]= new TH2F(Form("FR_SS_tauTau_Decay10_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay11[i]= new TH2F(Form("FR_SS_tauTau_Decay11_njet%i",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
    }
    
    FR_SS_electron_EB_unc=new TH1F("FR_SS_electron_EB_unc",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    FR_SS_electron_EE_unc=new TH1F("FR_SS_electron_EE_unc",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    FR_SS_muon_EB_unc=new TH1F("FR_SS_muon_EB_unc",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    FR_SS_muon_EE_unc=new TH1F("FR_SS_muon_EE_unc",";p_{T} [GeV];Fake Rate",_n_pT_bins-1,_pT_bins);
    for (int i = 0; i < 1; i++) {
        FR_SS_tauE_Decay0_unc[i]= new TH2F(Form("FR_SS_tauE_Decay0_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauE_Decay1_unc[i]= new TH2F(Form("FR_SS_tauE_Decay1_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauE_Decay10_unc[i]= new TH2F(Form("FR_SS_tauE_Decay10_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauE_Decay11_unc[i]= new TH2F(Form("FR_SS_tauE_Decay11_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay0_unc[i]= new TH2F(Form("FR_SS_tauMu_Decay0_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay1_unc[i]= new TH2F(Form("FR_SS_tauMu_Decay1_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay10_unc[i]= new TH2F(Form("FR_SS_tauMu_Decay10_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauMu_Decay11_unc[i]= new TH2F(Form("FR_SS_tauMu_Decay11_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay0_unc[i]= new TH2F(Form("FR_SS_tauTau_Decay0_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay1_unc[i]= new TH2F(Form("FR_SS_tauTau_Decay1_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay10_unc[i]= new TH2F(Form("FR_SS_tauTau_Decay10_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
        FR_SS_tauTau_Decay11_unc[i]= new TH2F(Form("FR_SS_tauTau_Decay11_njet%i_unc",i),";p_{T} [GeV];MET [GeV];Fake Rate",_n_pT_bins-1,_pT_bins,_n_MET_bins-1,_MET_bins);
    }
...
//         for (int i_MET_bin = 0; i_MET_bin < _n_MET_bins - 1; i_MET_bin++ ) {

//-----------------------------------ELECTRON------------------------------------------
        i_flav=Settings::ele;
        
        if (i_pT_bin > 0) { // electrons do not have 5 - 7 GeV bin 
        
        temp_NP = passing[Settings::Total][i_flav][0]->IntegralAndError(passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NP);
        temp_NF = failing[Settings::Total][i_flav][0]->IntegralAndError(failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NF);
        FR_SS_electron_EB->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_electron_EB->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
        
        temp_NP = passing[Settings::Total][i_flav][0]->IntegralAndError(passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NP);
        temp_NF = failing[Settings::Total][i_flav][0]->IntegralAndError(failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NF);
        FR_SS_electron_EE->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_electron_EE->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
        
        temp_NP = passing[Settings::Data][i_flav][0]->IntegralAndError(passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NP);
        temp_NF = failing[Settings::Data][i_flav][0]->IntegralAndError(failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NF);
        FR_SS_electron_EB_unc->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_electron_EB_unc->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
        
        temp_NP = passing[Settings::Data][i_flav][0]->IntegralAndError(passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NP);
        temp_NF = failing[Settings::Data][i_flav][0]->IntegralAndError(failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NF);
        FR_SS_electron_EE_unc->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_electron_EE_unc->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
        }
...
//-----------------------------------MUON------------------------------------------
        ...
        temp_NP = passing[Settings::Total][i_flav][0]->IntegralAndError(passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NP);
        temp_NF = failing[Settings::Total][i_flav][0]->IntegralAndError(failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NF);
        FR_SS_muon_EB->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_muon_EB->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
        
        temp_NP = passing[Settings::Total][i_flav][0]->IntegralAndError(passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NP);
        temp_NF = failing[Settings::Total][i_flav][0]->IntegralAndError(failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NF);
        FR_SS_muon_EE->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_muon_EE->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
        
        temp_NP = passing[Settings::Data][i_flav][0]->IntegralAndError(passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NP);
        temp_NF = failing[Settings::Data][i_flav][0]->IntegralAndError(failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 1, 1, temp_error_NF);
        FR_SS_muon_EB_unc->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_muon_EB_unc->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
        
        temp_NP = passing[Settings::Data][i_flav][0]->IntegralAndError(passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NP);
        temp_NF = failing[Settings::Data][i_flav][0]->IntegralAndError(failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][0]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, 1, 100, 2, 2, temp_error_NF);
        FR_SS_muon_EE_unc->SetBinContent(i_pT_bin+1,temp_NP/(temp_NP+temp_NF));
        FR_SS_muon_EE_unc->SetBinError(i_pT_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

    }
    
    for(int i_pT_bin = 0; i_pT_bin < _n_pT_bins - 1; i_pT_bin++ ) {
        if (i_pT_bin <= 2) continue; //hadronic taus do not have 5 - 20 GeV bin
        for (int i_MET_bin = 0; i_MET_bin < _n_MET_bins - 1; i_MET_bin++ ) {
            for (int i = 0; i < 1; i++) {
        
//-----------------------------------tauE------------------------------------------          
                i_flav=Settings::tauE;

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NF);
                FR_SS_tauE_Decay0[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay0[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NF);
                FR_SS_tauE_Decay1[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay1[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NF);
                FR_SS_tauE_Decay10[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay10[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NF);
                FR_SS_tauE_Decay11[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay11[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NF);
                FR_SS_tauE_Decay0_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay0_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NF);
                FR_SS_tauE_Decay1_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay1_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NF);
                FR_SS_tauE_Decay10_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay10_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NF);
                FR_SS_tauE_Decay11_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauE_Decay11_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

    //-----------------------------------tauMu------------------------------------------          
                i_flav=Settings::tauMu;

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NF);
                FR_SS_tauMu_Decay0[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay0[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NF);
                FR_SS_tauMu_Decay1[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay1[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NF);
                FR_SS_tauMu_Decay10[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay10[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NF);
                FR_SS_tauMu_Decay11[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay11[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NF);
                FR_SS_tauMu_Decay0_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay0_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NF);
                FR_SS_tauMu_Decay1_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay1_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NF);
                FR_SS_tauMu_Decay10_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay10_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NF);
                FR_SS_tauMu_Decay11_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauMu_Decay11_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

    //-----------------------------------tauTau------------------------------------------          
                i_flav=Settings::tauTau;

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NF);
                FR_SS_tauTau_Decay0[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay0[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NF);
                FR_SS_tauTau_Decay1[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay1[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NF);
                FR_SS_tauTau_Decay10[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay10[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Total][i_flav][i]->IntegralAndError(passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NP);
                temp_NF = failing[Settings::Total][i_flav][i]->IntegralAndError(failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Total][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Total][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NF);
                FR_SS_tauTau_Decay11[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay11[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 1, 1, temp_error_NF);
                FR_SS_tauTau_Decay0_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay0_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 2, 2, temp_error_NF);
                FR_SS_tauTau_Decay1_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay1_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 3, 3, temp_error_NF);
                FR_SS_tauTau_Decay10_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay10_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));

                temp_NP = passing[Settings::Data][i_flav][i]->IntegralAndError(passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),passing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NP);
                temp_NF = failing[Settings::Data][i_flav][i]->IntegralAndError(failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin]),failing[Settings::Data][i_flav][i]->GetXaxis()->FindBin(_pT_bins[i_pT_bin+1]) - 1, passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin]),passing[Settings::Data][i_flav][i]->GetYaxis()->FindBin(_MET_bins[i_MET_bin+1]) - 1, 4, 4, temp_error_NF);
                FR_SS_tauTau_Decay11_unc[i]->SetBinContent(i_pT_bin+1,i_MET_bin+1,temp_NP/(temp_NP+temp_NF));
                FR_SS_tauTau_Decay11_unc[i]->SetBinError(i_pT_bin+1,i_MET_bin+1,sqrt(pow((temp_NF/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NP,2) + pow((temp_NP/pow(temp_NF+temp_NP,2)),2)*pow(temp_error_NF,2)));
            }
        }
    }
    ...
    FR_SS_muon_EE->Write();
    for (int i = 0; i < 1; i++) {
        FR_SS_tauE_Decay0[i]->Write();
        FR_SS_tauE_Decay1[i]->Write();
        FR_SS_tauE_Decay10[i]->Write();
        FR_SS_tauE_Decay11[i]->Write();
        FR_SS_tauMu_Decay0[i]->Write();
        FR_SS_tauMu_Decay1[i]->Write();
        FR_SS_tauMu_Decay10[i]->Write();
        FR_SS_tauMu_Decay11[i]->Write();
        FR_SS_tauTau_Decay0[i]->Write();
        FR_SS_tauTau_Decay1[i]->Write();
        FR_SS_tauTau_Decay10[i]->Write();
        FR_SS_tauTau_Decay11[i]->Write();
    }
    ...
    for (int i = 0; i < 1; i++) {
        FR_SS_tauE_Decay0_unc[i]->Write();
        FR_SS_tauE_Decay1_unc[i]->Write();
        FR_SS_tauE_Decay10_unc[i]->Write();
        FR_SS_tauE_Decay11_unc[i]->Write();
        FR_SS_tauMu_Decay0_unc[i]->Write();
        FR_SS_tauMu_Decay1_unc[i]->Write();
        FR_SS_tauMu_Decay10_unc[i]->Write();
        FR_SS_tauMu_Decay11_unc[i]->Write();
        FR_SS_tauTau_Decay0_unc[i]->Write();
        FR_SS_tauTau_Decay1_unc[i]->Write();
        FR_SS_tauTau_Decay10_unc[i]->Write();
        FR_SS_tauTau_Decay11_unc[i]->Write();
    }
...
void SSmethod::SubtractWZ()
{
    ...
    {
        for (int i = 0; i < 3; i++) {
            passing[Settings::Total][i_flav][i]->Add(passing[Settings::WZ][i_flav][i], -1.);
            failing[Settings::Total][i_flav][i]->Add(failing[Settings::WZ][i_flav][i], -1.);
        }
...
void SSmethod::PlotFR()
{
    ...
    TCanvas *c_ele, *c_mu, *c_tauE_Decay0[3], *c_tauE_Decay1[3], *c_tauE_Decay10[3], *c_tauE_Decay11[3], *c_tauMu_Decay0[3], *c_tauMu_Decay1[3], *c_tauMu_Decay10[3], *c_tauMu_Decay11[3], *c_tauTau_Decay0[3], *c_tauTau_Decay1[3], *c_tauTau_Decay10[3], *c_tauTau_Decay11[3];
    c_ele = new TCanvas("FR_ele", "FR_ele", 600, 600);
    c_mu  = new TCanvas("FR_mu", "FR_mu", 600, 600);
    for (int i = 0; i < 1; i++) {
    c_tauE_Decay0[i] = new TCanvas(Form("FR_tauE_Decay0_njet%i",i), Form("FR_tauE_Decay0_njet%i",i), 600, 600);
    c_tauE_Decay1[i] = new TCanvas(Form("FR_tauE_Decay1_njet%i",i), Form("FR_tauE_Decay1_njet%i",i), 600, 600);
    c_tauE_Decay10[i] = new TCanvas(Form("FR_tauE_Decay10_njet%i",i), Form("FR_tauE_Decay10_njet%i",i), 600, 600);
    c_tauE_Decay11[i] = new TCanvas(Form("FR_tauE_Decay11_njet%i",i), Form("FR_tauE_Decay11_njet%i",i), 600, 600);
    c_tauMu_Decay0[i] = new TCanvas(Form("FR_tauMu_Decay0_njet%i",i), Form("FR_tauMu_Decay0_njet%i",i), 600, 600);
    c_tauMu_Decay1[i] = new TCanvas(Form("FR_tauMu_Decay1_njet%i",i), Form("FR_tauMu_Decay1_njet%i",i), 600, 600);
    c_tauMu_Decay10[i] = new TCanvas(Form("FR_tauMu_Decay10_njet%i",i), Form("FR_tauMu_Decay10_njet%i",i), 600, 600);
    c_tauMu_Decay11[i] = new TCanvas(Form("FR_tauMu_Decay11_njet%i",i), Form("FR_tauMu_Decay11_njet%i",i), 600, 600);
    c_tauTau_Decay0[i] = new TCanvas(Form("FR_tauTau_Decay0_njet%i",i), Form("FR_tauTau_Decay0_njet%i",i), 600, 600);
    c_tauTau_Decay1[i] = new TCanvas(Form("FR_tauTau_Decay1_njet%i",i), Form("FR_tauTau_Decay1_njet%i",i), 600, 600);
    c_tauTau_Decay10[i] = new TCanvas(Form("FR_tauTau_Decay10_njet%i",i), Form("FR_tauTau_Decay10_njet%i",i), 600, 600);
    c_tauTau_Decay11[i] = new TCanvas(Form("FR_tauTau_Decay11_njet%i",i), Form("FR_tauTau_Decay11_njet%i",i), 600, 600);
    }
//-----------------------------------ELECTRON------------------------------------------          
    FR_SS_electron_EB->SetLineColor(kBlue);
    FR_SS_electron_EB->SetStats(0);
    ...
    FR_SS_electron_EB_unc->SetStats(0);
    ...
    FR_SS_electron_EE_unc->SetStats(0);
...
//-----------------------------------MUON------------------------------------------          
    ...
    for (int i = 0; i < 1; i++) {
//-----------------------------------tauE------------------------------------------          
        FR_SS_tauE_Decay0[i]->SetTitle(Form("decay mode 0, njet %i",i));
        FR_SS_tauE_Decay0[i]->SetStats(0);
        FR_SS_tauE_Decay1[i]->SetTitle(Form("decay mode 1, njet %i",i));
        FR_SS_tauE_Decay1[i]->SetStats(0);
        FR_SS_tauE_Decay10[i]->SetTitle(Form("decay mode 10, njet %i",i));
        FR_SS_tauE_Decay10[i]->SetStats(0);
        FR_SS_tauE_Decay11[i]->SetTitle(Form("decay mode 11, njet %i",i));
        FR_SS_tauE_Decay11[i]->SetStats(0);

//-----------------------------------tauMu------------------------------------------          
        FR_SS_tauMu_Decay0[i]->SetTitle(Form("decay mode 0, njet %i",i));
        FR_SS_tauMu_Decay0[i]->SetStats(0);
        FR_SS_tauMu_Decay1[i]->SetTitle(Form("decay mode 1, njet %i",i));
        FR_SS_tauMu_Decay1[i]->SetStats(0);
        FR_SS_tauMu_Decay10[i]->SetTitle(Form("decay mode 10, njet %i",i));
        FR_SS_tauMu_Decay10[i]->SetStats(0);
        FR_SS_tauMu_Decay11[i]->SetTitle(Form("decay mode 11, njet %i",i));
        FR_SS_tauMu_Decay11[i]->SetStats(0);

//-----------------------------------tauTau------------------------------------------          
        FR_SS_tauTau_Decay0[i]->SetTitle(Form("decay mode 0, njet %i",i));
        FR_SS_tauTau_Decay0[i]->SetStats(0);
        FR_SS_tauTau_Decay1[i]->SetTitle(Form("decay mode 1, njet %i",i));
        FR_SS_tauTau_Decay1[i]->SetStats(0);
        FR_SS_tauTau_Decay10[i]->SetTitle(Form("decay mode 10, njet %i",i));
        FR_SS_tauTau_Decay10[i]->SetStats(0);
        FR_SS_tauTau_Decay11[i]->SetTitle(Form("decay mode 11, njet %i",i));
        FR_SS_tauTau_Decay11[i]->SetStats(0);
    }
    ...
    FR_SS_electron_EB->SetMaximum(0.35);
    FR_SS_electron_EB->Draw("E");
    FR_SS_electron_EE->Draw("Esame");
    FR_SS_electron_EB_unc->Draw("Esame");
    FR_SS_electron_EE_unc->Draw("Esame");
    ...
    FR_SS_muon_EB->SetMaximum(0.35);
    FR_SS_muon_EB->Draw("E");
    FR_SS_muon_EE->Draw("Esame");
    FR_SS_muon_EB_unc->Draw("Esame");
    FR_SS_muon_EE_unc->Draw("Esame");
    ...
    for (int i = 0; i < 1; i++) {
        c_tauE_Decay0[i]->cd();
        lumi->set_lumi(c_tauE_Decay0[i], _lumi, 0);
        FR_SS_tauE_Decay0[i]->Draw("ColzText");
        SavePlots(c_tauE_Decay0[i], Form("Plots/FR_SS_tauE_Decay0_njet%i",i));
        c_tauE_Decay1[i]->cd();
        lumi->set_lumi(c_tauE_Decay1[i], _lumi, 0);
        FR_SS_tauE_Decay1[i]->Draw("ColzText");        
        SavePlots(c_tauE_Decay1[i], Form("Plots/FR_SS_tauE_Decay1_njet%i",i));
        c_tauE_Decay10[i]->cd();
        lumi->set_lumi(c_tauE_Decay10[i], _lumi, 0);
        FR_SS_tauE_Decay10[i]->Draw("ColzText");
        SavePlots(c_tauE_Decay10[i], Form("Plots/FR_SS_tauE_Decay10_njet%i",i));
        c_tauE_Decay11[i]->cd();
        lumi->set_lumi(c_tauE_Decay11[i], _lumi, 0);
        FR_SS_tauE_Decay11[i]->Draw("ColzText");
        SavePlots(c_tauE_Decay11[i], Form("Plots/FR_SS_tauE_Decay11_njet%i",i));
        
        c_tauMu_Decay0[i]->cd();
        lumi->set_lumi(c_tauMu_Decay0[i], _lumi, 0);
        FR_SS_tauMu_Decay0[i]->Draw("ColzText");
        SavePlots(c_tauMu_Decay0[i], Form("Plots/FR_SS_tauMu_Decay0_njet%i",i));
        c_tauMu_Decay1[i]->cd();
        lumi->set_lumi(c_tauMu_Decay1[i], _lumi, 0);
        FR_SS_tauMu_Decay1[i]->Draw("ColzText");        
        SavePlots(c_tauMu_Decay1[i], Form("Plots/FR_SS_tauMu_Decay1_njet%i",i));
        c_tauMu_Decay10[i]->cd();
        lumi->set_lumi(c_tauMu_Decay10[i], _lumi, 0);
        FR_SS_tauMu_Decay10[i]->Draw("ColzText");
        SavePlots(c_tauMu_Decay10[i], Form("Plots/FR_SS_tauMu_Decay10_njet%i",i));
        c_tauMu_Decay11[i]->cd();
        lumi->set_lumi(c_tauMu_Decay11[i], _lumi, 0);
        FR_SS_tauMu_Decay11[i]->Draw("ColzText");
        SavePlots(c_tauMu_Decay11[i], Form("Plots/FR_SS_tauMu_Decay11_njet%i",i));
        
        c_tauTau_Decay0[i]->cd();
        lumi->set_lumi(c_tauTau_Decay0[i], _lumi, 0);
        FR_SS_tauTau_Decay0[i]->Draw("ColzText");
        SavePlots(c_tauTau_Decay0[i], Form("Plots/FR_SS_tauTau_Decay0_njet%i",i));
        c_tauTau_Decay1[i]->cd();
        lumi->set_lumi(c_tauTau_Decay1[i], _lumi, 0);
        FR_SS_tauTau_Decay1[i]->Draw("ColzText");        
        SavePlots(c_tauTau_Decay1[i], Form("Plots/FR_SS_tauTau_Decay1_njet%i",i));
        c_tauTau_Decay10[i]->cd();
        lumi->set_lumi(c_tauTau_Decay10[i], _lumi, 0);
        FR_SS_tauTau_Decay10[i]->Draw("ColzText");
        SavePlots(c_tauTau_Decay10[i], Form("Plots/FR_SS_tauTau_Decay10_njet%i",i));
        c_tauTau_Decay11[i]->cd();
        lumi->set_lumi(c_tauTau_Decay11[i], _lumi, 0);
        FR_SS_tauTau_Decay11[i]->Draw("ColzText");
        SavePlots(c_tauTau_Decay11[i], Form("Plots/FR_SS_tauTau_Decay11_njet%i",i));
    }
}
//===============================================================
...
//===============================================================
void SSmethod::RemoveNegativeBins3D(TH3F *h)
{
    for (int i_bin_x = 1; i_bin_x <= h->GetXaxis()->GetNbins(); i_bin_x++)
    {
        for (int i_bin_y = 1; i_bin_y <= h->GetYaxis()->GetNbins(); i_bin_y++)
        {
            for (int i_bin_z = 1; i_bin_z <= h->GetZaxis()->GetNbins(); i_bin_z++)
            {
                if( h->GetBinContent(i_bin_x,i_bin_y,i_bin_z) < 0.) h->SetBinContent(i_bin_x,i_bin_y,i_bin_z,0);
            }
        }
    }
}
...
//===============================================================
void SSmethod::Set_MET_binning(int size, float *bins)
{
   _n_MET_bins = size;
...
{
   TString input_file_name_=input_file_name.ReplaceAll("HTauTauHMuMu.root","");
   if (abs(LepLepId->at(ileg))!=15) {
      cout<<"Only applicable to pdgId = 15, not "<<ileg<<endl;
      return 6;
   }
   if ( input_file_name_.Contains("WZTo") || input_file_name_.Contains("ZZ") || input_file_name_.Contains("H") || input_file_name_.Contains("ggTo") ) {
...
float SSmethod::calculate_TauIDSF_OneLeg(short DM, float pt, float eta, int genmatch, int flav, bool tight)
{
    eta=fabs(eta);
    if (tight) {
        if (genmatch==6)
...
float SSmethod::calculate_TauIDSF(TString input_file_name, bool tight)
{
   float SF = 1;
/*   TString WP[3];
   if (Z2Flav==-165) {
...
//=========================================================================================================
TLegend* SSmethod::CreateLegend_FR( string position, TH1F *EB_unc, TH1F *EB_cor,TH1F *EE_unc,TH1F *EE_cor )
{
   TLegend *leg;
...
//=========================================================================================================
TLegend* SSmethod::CreateLegend_FR_tau( string position, TH2F *Decay0_unc, TH2F *Decay0_cor,TH2F *Decay1_unc,TH2F *Decay1_cor, TH2F *Decay10_unc, TH2F *Decay10_cor,TH2F *Decay11_unc,TH2F *Decay11_cor )
{
   TLegend *leg;
   leg = new TLegend( .64, .65, .97, .9 );
   if(position == "right") leg = new TLegend( .64, .65, .97, .9 );
   ...
   leg->AddEntry( Decay0_unc, "decay mode 0 uncorrected", "l" );
   leg->AddEntry( Decay0_cor, "decay mode 0 corrected","l");
   leg->AddEntry( Decay1_unc, "decay mode 1 uncorrected", "l" );
   leg->AddEntry( Decay1_cor, "decay mode 1 corrected", "l" );
   leg->AddEntry( Decay10_unc, "decay mode 10 uncorrected", "l" );
   leg->AddEntry( Decay10_cor, "decay mode 10 corrected","l");
   leg->AddEntry( Decay11_unc, "decay mode 11 uncorrected", "l" );
   leg->AddEntry( Decay11_cor, "decay mode 11 corrected", "l" );   
   return leg;
}
//=========================================================================================================
