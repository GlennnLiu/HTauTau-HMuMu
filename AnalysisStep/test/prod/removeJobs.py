...
for chunk in chunkList:
	...
	if os.path.exists("{0}{1}/log.txt".format(eosPath,chunk)):
		...
		os.system('grep "A fatal system signal has occurred" {0}{1}/log.txt > tmp1.txt'.format(eosPath,chunk))
                ...
                os.system('zgrep "A fatal system signal has occurred" {0}{1}/log.txt.gz > tmp1.txt'.format(eosPath,chunk))
	else:
		remove=True
	
	if len(open("tmp.txt").readlines())==0:
		remove=True
	if len(open("tmp1.txt").readlines())>0:
		remove=True
